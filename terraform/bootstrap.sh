...
proof_script="const {S3Client,PutObjectCommand}=require('@aws-sdk/client-s3');new S3Client({}).send(new PutObjectCommand({Bucket:process.env.S3_BUCKET,Key:process.env.STORAGE_PROOF_KEY,Body:'DingoDocs Fargate storage proof\\n',ContentType:'text/plain'})).then(()=>console.log('stored '+process.env.STORAGE_PROOF_KEY)).catch(e=>{console.error(e);process.exit(1)})"
...
Not yet recorded. Run \`DINGODOCS_ALLOW_DESTROY=yes ./destroy.sh <same Terraform arguments>\`.
EOF
