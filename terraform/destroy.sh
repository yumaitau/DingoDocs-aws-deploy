...
if [[ "${DINGODOCS_ALLOW_DESTROY:-}" != "yes" ]]; then
  echo "Refusing destroy. Set DINGODOCS_ALLOW_DESTROY=yes after checking account, region, and workspace." >&2
  exit 1
fi
...
echo "Destroying DingoDocs resources in AWS account ${account_id}, region ${region}."
